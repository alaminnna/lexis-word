  ...
  return (
    <div>
      ...
      <textarea
        ...
        className="min-h-[96px] w-full rounded-xl border border-line bg-paper px-4 py-3 text-lg leading-relaxed transition-calm placeholder:text-ink-faint focus:border-accent focus:outline-none"
        autoCorrect="off"
        autoCapitalize="off"
        spellCheck={false}
        autoComplete="off"
      />
      <p id={`ws-hint-${wordId}-${task}`} className="mt-1 text-sm text-ink-faint">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <Button disabled={!ready} onClick={submit}>{submitLabel}</Button>
        {restored && (
          <button
            onClick={() => {
              clearDraft(wordId, task);
            ...
            className="inline-flex min-h-[44px] cursor-pointer items-center px-3 text-sm text-ink-faint hover:text-ink"
          >
            Discard draft
          </button>
        )}
